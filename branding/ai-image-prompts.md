# Brite Shopping — AI Image Generation Prompts

Use these prompts with Midjourney, DALL-E, or Ideogram to generate brand assets.

Brand color: **Teal #0a7ea4** | Light: #4dc9f6 | Dark: #07556f
...
**Requirements:**
- 1024x1024 PNG
- No transparency/alpha (required for iOS App Store)
- Simple enough to read at 60x60 pixels
- Will be auto-rounded by iOS/Android

---

## Splash Screen Icon

> A minimal centered icon for a mobile app splash screen. A teal (#0a7ea4) shopping cart or price tag symbol on a clean white background. Simple, elegant, Apple-style. Optionally include small "Brite Shopping" text beneath in gray. Vector-style flat design. The icon should be roughly 200x200 pixels centered in a 1284x2778 canvas.

**Requirements:**
- Source icon: 1024x1024 PNG (Expo will center it)
- Background color set in app.json (white #ffffff)
...
1. **Midjourney**: Append `--ar 1:1` for icons, `--ar 16:9` for hero illustrations
2. **DALL-E**: Specify "flat design, vector style, no 3D, no photorealism"
3. **Ideogram**: Good for text-in-image (logo with "Brite Shopping" text)
4. Generate multiple variants and pick the cleanest one
5. After generation, upscale to required dimensions with an AI upscaler if needed
6. For the app icon, test at small sizes (render at 60x60) to ensure readability
